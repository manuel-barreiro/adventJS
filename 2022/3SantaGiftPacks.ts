...
function distributeGifts({  packOfGifts, reindeers } : { packOfGifts: string[], reindeers: string[] } ) {
  // Almacenar en variable el peso del paquete
  const weightOfGifts = packOfGifts.reduce(
  (accumulator, gift) => accumulator + gift.length,
  0,
);

// Almacenar en variable la capacidad de carga
  const reindeersCap = reindeers.reduce(
  (accumulator, reindeer) => accumulator + 2 * reindeer.length,
  0,
);
  
  return Math.floor(reindeersCap / weightOfGifts)
}